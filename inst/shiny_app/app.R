...
list_users <- function(persona_dir) {
    lapply(biodt.recreation::list_csv_files(persona_dir), tools::file_path_sans_ext)
}
...
server <- function(persona_dir = NULL, data_dir = NULL) {
    ...
    if (is.null(persona_dir)) {
       persona_dir <- system.file("extdata", package = "biodt.recreation", mustWork = TRUE)
    }
    ...
    server <- function(input, output, session) {
        ...
        observeEvent(input$loadButton, {
            ...
            updateSelectInput(
                session,
                "loadPersonaSelect",
                choices = biodt.recreation::list_personas_in_file(
        ...
        observeEvent(input$updateButton, {
            ...
            msg <- capture.output(
                valid_persona <- biodt.recreation::is_valid_persona(persona),
                type = "message"
            )
